...
def _collect_rest_area_reference() -> dict[str, Any]:
    settings = _settings()
    return asyncio.run(_run_with_session(settings, RestAreaCollectionService(settings).collect_reference))
...
def collect_rest_area_reference() -> dict[str, Any]:
    return _collect_rest_area_reference()
...
definitions = Definitions(
    ...
    schedules=[
        ...
        ScheduleDefinition(job=rest_area_reference_collection_job, cron_schedule="0 3 */3 * *", execution_timezone="Asia/Seoul", default_status=DefaultScheduleStatus.RUNNING),
    ],
)
