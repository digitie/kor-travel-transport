...
- 추가 실데이터 검증에서 소청도의 10일 API를 날짜별 두 번씩 읽어 동일한 저장 응답을
  확인했고, 인천의 오늘 운항 50건과 마지막 보관일의 빈 응답을 1440px/390px 화면에서
  조회했다. 가로 넘침 없이 날짜 선택과 실제 응답 표시를 확인했다. 기존 배편 탭은 지도용
  장소 API를 사용해 좌표가 연결된 8개 항구만 표시한다. DB 수집 대상 749개와 화면의
  8개를 혼동하지 않도록, 좌표 없는 항구까지 검색하는 목록 계약을 별도 후속 항목으로 남겼다.
- 관리 UI 실행 후보 `6613515`를 n150에 배포한 뒤 HTTPS live E2E 279개가 모두 통과했다
  (1.4분). 로그인·로그아웃, 늦은 캐시 저장 재현, 공개/관리 API 행렬, 모바일·지도·날짜
  선택 및 쓰기/CSRF 경계를 검증했다. James/Popper는 이 수정에 P0/P1/P2 없음으로
  재리뷰했다. 이 커밋 이후 변경은 검증 기록 문서뿐이며 backend/code-server는 검증된
  `0d7d292`, 관리 UI는 `6613515`를 유지한다.
- 재검증 중 지도 장소 선택은 주유소 응답이 끝나기 전 기본 5초 assertion timeout에
  걸린 실행이 한 번 있었다(274개 통과·1개 실패·4개 미실행). 주유소 bbox API 재측정은
  HTTP 200/0.93초, 동일 코드의 지도 단독 검증 및 전체 279개 재실행은 통과했다.
  테스트 시간 제한이나 검증 조건은 완화하지 않았다. 호스트 I/O 부하가 높을 때의 지도
  대기시간은 기존 성능 후속 항목에서 계속 관찰한다.
- n150 첫 278개 live E2E는 로그아웃 캐시 회귀 1건을 발견했다(277개 통과). 로그아웃
  form 제출 시 캐시를 지워도 이전 문서의 늦은 통계 응답이 다시 저장했다. 로그인 문서가
  열린 뒤에도 캐시를 제거하도록 보완하고, submit 뒤 비동기 캐시 저장을 주입하는
  Playwright 재현 테스트를 추가했다. 문서 리뷰의 P2에 따라 구현 완료와 전국 수집·최종
  CI·live E2E·머지의 대기 상태를 완료 아카이브와 인수인계에 직접 구분했다.
- n150 디스크 정리 후 여유 공간은 최종 배포 중 측정에서 109GiB였다. 중지된 컨테이너와
  미사용 이미지·빌드 캐시를 정리했으며 볼륨은 삭제하지 않았다. 기존 parking-radar
  프론트엔드는 재배포하지 않았다. 실행 코드 후보 `0d7d292`의 backend와 Dagster
  code-server를 배포하고 PostgreSQL migration `0013` 및 배편 수집 스케줄 활성화를 확인했다.
  James/Popper 독립 최종 리뷰는 각각 P0/P1/P2 없음이었다.
- 최신 소스를 mount하고 현재 provider 의존성을 설치한 WSL Docker의 backend 전체 검증은
  175개 통과·1개 건너뜀, 관리 UI는 21개 통과했다. 이전 backend 이미지에는 `datagokr`가
  없었으므로 기존 이미지의 테스트 결과를 최신 코드 검증으로 재사용하지 않았다.
  프론트엔드 컨테이너 검증은 별도 Linux 의존성 볼륨에서 설치해 musl/glibc 차이를 제거했다.
- 운영 초기 배편 run `16d839e6-e0e4-4d61-ad35-7d2a5c739614`에서 소청도의 10일 스냅샷과
  운항 149건을 확인했다. 공개 HTTPS API로 각 날짜를 두 번씩 읽어 응답과 수집시각의
  동일성을 검증했다. 첫 연결 이후 두 번 읽는 시간은 25~70ms였고, 기간 밖 날짜는
  `422`였다. 749개 항구 전체가 채워졌다고 해석해서는 안 되며 초기 수집은 제한된
  호출 예산으로 다음 실행에서도 이어진다. UI·전체 live E2E 및 머지 증거는 PR #40에 기록한다.
- 여객선 시간표 정책을 실시간 전용에서 PostgreSQL 저장 우선으로 변경했다. 새
  ...
  SQLite 단위 테스트로 고정했다.
- 적대적 리뷰에서 확인한 최장 응답시간 경계를 반영해, 시간표 job 기본 호출 예산을 280건으로
  낮추고 provider 보호 간격과 API timeout을 포함한 3시간 30분 상한 검증을 설정에 추가했다.
  자정을 넘겨 열린 배편 탭도 과거 운항일을 요청하지 않도록 요청 직전에 KST 오늘로 보정한다.
