...
export function TransportMap() {
  ...
  const bounds = useMemo<[number, number, number, number] | undefined>(() => {
    ...
    return points.reduce<[number, number, number, number]>((current, point) => [Math.min(current[0], point.lngLat[0]), Math.min(current[1], point.lngLat[1]), Math.max(current[2], point.lngLat[0]), Math.max(current[3], point.lngLat[1])], [points[0].lngLat[0], points[0].lngLat[1], points[0].lngLat[0], points[0].lngLat[1]]);
  }, [points]);
  ...
  return <section className="transport-map-layout" aria-label="교통 장소 지도">
    <VWorldMapView apiKey={process.env.NEXT_PUBLIC_VWORLD_API_KEY ?? ""} bbox={selected ? undefined : bounds} cameraTarget={selected ? { center: [selected.longitude, selected.latitude], zoom: 11 } : undefined} center={[127.8, 36.2]} className="transport-map-canvas" fallback={() => <p className="loading">VWorld 지도 키가 설정되지 않아 지도 대신 저장된 장소 목록을 표시합니다.</p>} geolocate={false} layerType="Base" lazy loadingSkeleton={<p className="loading">지도를 준비하는 중입니다…</p>} minZoom={5} navigation scale semanticZoomThreshold={11} zoom={6}>
    ...
    <aside className="transport-map-detail" aria-live="polite">
      ...
      {selected ? <><p className="eyebrow">{placeKindLabel(selected.kind)}</p><h2>{selected.name}</h2><p>{selected.line_names.length ? `운행 노선: ${selected.line_names.join(", ")}` : selected.address ?? "상세 정보는 지도 팝업에서 확인할 수 있습니다."}</p></> : <><h2>교통 장소</h2><p>{message || `주유소·역·항구 ${points.length.toLocaleString("ko-KR")}곳을 표시합니다. 지도 또는 목록을 선택하면 상세 정보를 봅니다.`}</p></>}
    </aside>
  </section>;
}
