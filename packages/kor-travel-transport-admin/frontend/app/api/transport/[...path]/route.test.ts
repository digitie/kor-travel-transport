...
describe("GET /api/transport/[...path]", () => {
  ...
  it("허용한 읽기 path의 query만 backend /v1에 전달한다", async () => {
    const fetchMock = vi.fn().mockResolvedValue(new Response(JSON.stringify({ items: [] }), { status: 200, headers: { "content-type": "application/json" } })); vi.stubGlobal("fetch", fetchMock);
    ...
    expect(response.status).toBe(200); expect(String(fetchMock.mock.calls[0][0])).toContain("/v1/transport/highways/traffic?route_no=1&route_no=10");
  });
});
