...
test.describe("인증된 관리 proxy 행렬과 UI", () => {
  ...
  test("지도는 VWorld 타일과 저장 장소 API를 읽고, 실시간 항구 시간표를 자동 호출하지 않는다", async ({ browser }) => {
    ...
    expect([...mapPlaceRequests.values()].every((url) => url.searchParams.get("limit") === "5000" && ["min_longitude", "min_latitude", "max_longitude", "max_latitude"].every((key) => url.searchParams.has(key)))).toBe(true);
